...
### 예시 2: 복잡한 집계 (Native Query)

**요구사항**: JSONB 필드를 사용한 PostgreSQL 전용 쿼리

```java
// GameLogRepositoryCustom.java
public interface GameLogRepositoryCustom {
    List<ErrorCodeStatistics> getTopErrorCodes(UUID projectId, int limit);
}

// GameLogRepositoryImpl.java
public class GameLogRepositoryImpl implements GameLogRepositoryCustom {

    private final EntityManager entityManager;

    public GameLogRepositoryImpl(EntityManager em) {
        this.entityManager = em;
    }

    @Override
    public List<ErrorCodeStatistics> getTopErrorCodes(UUID projectId, int limit) {
        String sql =
                """
                SELECT
                    attributes->>'errorCode' AS error_code,
                    COUNT(*) AS occurrence_count
                FROM game_log
                WHERE project_id = :projectId
                  AND attributes->>'errorCode' IS NOT NULL
                GROUP BY attributes->>'errorCode'
                ORDER BY COUNT(*) DESC
                LIMIT :limit
                """;
        ...
        return entityManager
                ...
                .map(
                        tuple ->
                                new ErrorCodeStatistics(
                                        (String) tuple.get("error_code"),
                                        ((Number) tuple.get("occurrence_count")).longValue()))
...
### 예시 3: Bulk Update (JPQL)

**요구사항**: 특정 조건의 이슈를 일괄 RESOLVED 처리

```java
// IssueRepositoryCustom.java
public interface IssueRepositoryCustom {
    int bulkResolveIssues(UUID projectId, OffsetDateTime before);
}
...
public class IssueRepositoryImpl implements IssueRepositoryCustom {
    ...
    @Override
    @Modifying
    public int bulkResolveIssues(UUID projectId, OffsetDateTime before) {
        String jpql =
                """
                UPDATE Issue i
                SET i.status = :status, i.updatedAt = :now
                WHERE i.projectId = :projectId
                  AND i.lastOccurredAt < :before
                  AND i.status = :openStatus
                """;

        return entityManager
                .createQuery(jpql)
                .setParameter("status", IssueStatus.RESOLVED)
