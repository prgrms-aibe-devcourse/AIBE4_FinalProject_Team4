...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (error === "email_conflict" && existingProvider) {
        if (!ALLOWED_PROVIDERS.includes(existingProvider)) return;
...
function initConflictStep(existingProvider) {
    ...
    function buildProviderBtn(provider, label, isLink, href) {
        ...
        if (isLink) {
            ...
            el.type = "button";
            el.id = "conflict-new-account-btn";
        }
        el.className = cls;
    ...
    if (buttonsEl) {
        ...
        buttonsEl.replaceChildren(
            buildProviderBtn(existingProvider, existingLabel, true, `/oauth2/authorization/${existingProvider}`),
            buildProviderBtn(newProvider, newLabel, false, ""),
        );
