 ...
 *
 * <p>- checkMultipart() : MaxUploadSizeExceededException (멀티파트 파싱 단계) - getHandler() :
 * HttpRequestMethodNotSupportedException, NoResourceFoundException (핸들러 탐색 단계)
 *
 * <p>이 시점에는 handler = null 이므로 어노테이션 제한이 있는 @ControllerAdvice는 적용되지 않는다. 따라서 제한 없는 별도 어드바이스로 분리하여
 * 처리한다.
 *
 * <p>Accept 헤더를 확인하여 JSON(API) 또는 HTML(View) 응답을 분기한다.
 */
@Slf4j
@ControllerAdvice
public class GlobalPreDispatchExceptionHandler {
    ...
    public Object handleMaxUploadSizeExceeded(
        ...
        log.warn(
                "MaxUploadSizeExceededException [{}]: {}", request.getRequestURI(), e.getMessage());
