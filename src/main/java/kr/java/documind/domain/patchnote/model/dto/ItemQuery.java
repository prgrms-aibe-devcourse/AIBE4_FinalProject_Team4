 ...
 *
 * <p>{@link kr.java.documind.domain.patchnote.service.ItemQueryBuilder}가 PendingItem 하나에서
 * 생성하며, {@link HybridVectorSearchRepositoryCustom}의
 * 항목별 검색 API에 전달된다.
 *
 * <p>이전의 모든 항목 요약을 하나로 합치던 방식과 달리, 항목 단위로 독립적인 쿼리를 보유하여
 * 노이즈를 최소화하고 검색 정확도를 높인다.
 *
 * @param sourceId  이 항목의 source_id (vector_store 필터용)
 * @param itemRef   항목의 소스 REF (로깅·추적용, 예: "ISSUE-42", "DOC-17-0")
 * @param keyword   핵심 토큰을 공백으로 이어 붙인 문자열
 *                  — {@code similarity(content, :keyword)} 스코어링에 사용
 * @param tokens    개별 핵심 토큰 목록
 *                  — OR-based {@code LIKE} 조건 생성에 사용 (bigm GIN 인덱스 활용)
 * @param tsquery   PostgreSQL {@code to_tsquery()} 식 (예: {@code "수정 | 추가 | 변경"})
 *                  — {@code ts_rank} 스코어링에 사용. 유효 토큰이 없으면 null
