 ...
 *
 * <p>하나의 {@code PendingItem}에 대한 모든 RAG 증거를 구조화하여 보유한다. {@link PatchNotePromptService}가 이 목록으로부터
 * LLM에 전달할 증거 블록 텍스트를 렌더링한다.
 *
 * <p>{@code allowedSourceRefs}는 LLM이 이 항목의 응답에서 {@code sourceRefs}로 인용할 수 있는 REF 목록이다. 현재는 항목 자신의
 * ref만 허용하며, 추후 관련 소스 cross-ref 확장 시 사용한다.
 *
