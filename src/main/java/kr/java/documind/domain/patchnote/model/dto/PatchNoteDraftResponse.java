 ...
 *
 * <p>모든 LLM 출력은 이 스키마를 따르는 JSON 문자열이어야 한다.
 * {@code PatchNoteDraftService}가 LLM 응답을 파싱하여 이 DTO로 역직렬화한다.
 *
 * <p>섹션 순서는 항상 {@code NEW → CHANGE → FIX → MAINTENANCE}이며,
 * 해당 분류의 항목이 없으면 그 섹션은 {@code sections} 배열에 포함되지 않는다.
 *
