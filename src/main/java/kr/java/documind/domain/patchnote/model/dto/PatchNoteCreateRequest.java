 ...
 * 패치노트 저장 요청 DTO.
 *
 * <p>SSE 스트리밍 완료 후 프론트엔드가 {@code done} 이벤트의 {@code cleanedContent}를
 * 그대로 담아 전송한다. 선택된 pending_item ID 목록을 함께 전달하면 해당 항목이 COMPLETED 처리된다.
 *
 * @param title        패치노트 제목
...
public record PatchNoteCreateRequest(
        ...
        @Schema(description = "COMPLETED 처리할 PendingItem ID 목록", example = "[1, 2, 3]")
                @NotNull
                List<Long> itemIds,
