 ...
 *
 * <p>이전의 단일 {@code contextText} 문자열 대신, 항목별로 구조화된 {@link ItemContext} 목록을 보유한다.
 * {@link kr.java.documind.domain.patchnote.service.PatchNotePromptService}가
 * 이 목록으로부터 LLM 시스템 프롬프트의 증거 블록 텍스트를 렌더링한다.
 *
 * <p>소스 REF 맵({@code sourceRefMap})과 참조 목록({@code sourceRefs})은 LLM 응답 검증 및
 * SSE sources 이벤트 전송에 계속 사용된다.
 *
