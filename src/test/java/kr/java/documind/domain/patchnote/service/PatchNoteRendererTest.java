    ...
    class Render {
        ...
        void render_단일섹션_H2헤더불릿포인트포함() {
            ...
            PatchNoteDraftResponse resp =
                    response(section("FIX", item("전투 밸런스 버그 수정", "ISSUE-42")));
        ...
        void render_빈텍스트항목_스킵() {
            // Given
            PatchNoteDraftResponse resp =
                    response(section("NEW", item(""), item("   "), item("유효한 항목")));
            ...
            // Then — 유효한 항목 하나만 렌더링
            long bulletCount =
                    result.cleanedContent().lines().filter(line -> line.startsWith("- ")).count();
        ...
        void render_sourceRefs없는항목_태그없이렌더링() {
            // Given
            PatchNoteDraftResponse resp = response(section("NEW", item("태그 없는 항목")));
